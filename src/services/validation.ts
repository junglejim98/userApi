...
export function pickRegisterBody(body: any) {
    const errors: { path: string; message: string }[] = [];

    if (!isNonEmptyString(body.firstName)) errors.push({ path: 'firstName', message: 'Обязательное поле'});
    if (!isNonEmptyString(body.lastName)) errors.push({ path: 'lastName', message: 'Обязательно поле'});

    let middleName: string | undefined = undefined;

    if (typeof body.middleName === 'string' && body.middleName.trim() !== '') {
        if (!isNonEmptyString(body.middleName)) errors.push({ path: 'middleName', message: 'Некорректное значение'});
    ...
    if (errors.length) {
        const err: any = new Error('Ошибка валидации');
        err.status = 400;
